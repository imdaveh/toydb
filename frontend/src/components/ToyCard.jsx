        ...
        {expanded && (
          <>
            ...
            <div className="mt-2 text-xs text-toydb-slate">
              ...
              {(() => {
                const accessories = Array.isArray(toy.accessories) ? toy.accessories : []
                const neededAccessories = accessories.filter(item => !item.has_accessory)

                if (neededAccessories.length > 0) {
                  return (
                    <ul className="mt-1 list-disc pl-5">
                      {neededAccessories.map(item => (
