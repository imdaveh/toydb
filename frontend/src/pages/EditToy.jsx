  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-4 bg-toydb-white border border-toydb-border rounded-xl shadow-sm space-y-2">
        {/* 1st row: Name */}
        <div>
          <label htmlFor="toy-name" className="block text-sm font-medium text-toydb-navy mb-1">Name</label>
        ...
        <div className="flex gap-2">
          <div className="w-24">
            <label htmlFor="toy-year" className="block text-sm font-medium text-toydb-navy mb-1">Year</label>
          ...
          <div className="flex-1">
            <label htmlFor="toy-manufacturer" className="block text-sm font-medium text-toydb-navy mb-1">Manufacturer</label>
        ...
        {/* 3rd row: Toyline */}
        <div>
          <label htmlFor="toy-toyline" className="block text-sm font-medium text-toydb-navy mb-1">Toyline</label>
        ...
        {/* 4th row: Series */}
        <div>
          <label htmlFor="toy-series" className="block text-sm font-medium text-toydb-navy mb-1">Series</label>
          <input id="toy-series" value={form.series || ''} onChange={e=>setForm({...form, series: e.target.value})} className="w-full p-2 border rounded" />
        </div>

        {/* 5th row: Sub-series */}
        <div>
          <label htmlFor="toy-subseries" className="block text-sm font-medium text-toydb-navy mb-1">Sub-series</label>
        ...
        {/* 6th row: Accessories */}
        <div>
          <label htmlFor="toy-accessories" className="block text-sm font-medium text-toydb-navy mb-1">Accessories</label>
        ...
        {/* 7th row: Notes */}
        <div>
          <label htmlFor="toy-notes" className="block text-sm font-medium text-toydb-navy mb-1">Notes</label>
          <textarea id="toy-notes" value={form.notes || ''} onChange={e=>setForm({...form, notes: e.target.value})} className="w-full p-2 border rounded" />
        </div>

        {/* 8th row: Condition */}
        <div>
          <label htmlFor="toy-condition" className="block text-sm font-medium text-toydb-navy mb-1">Condition</label>
        ...
        <div className="flex gap-2">
          <div className="w-32">
            <label htmlFor="toy-cost" className="block text-sm font-medium text-toydb-navy mb-1">Price</label>
          ...
          <div className="flex-1">
            <label htmlFor="toy-source" className="block text-sm font-medium text-toydb-navy mb-1">Source</label>
          ...
          <div className="grid grid-cols-3 gap-2 mt-2">
            {toy.photos && toy.photos.length ? toy.photos.map(p => (
              <div key={p.id} className="relative">
                ...
                <button onClick={()=>deletePhoto(p.id)} className="absolute top-1 right-1 bg-toydb-danger text-toydb-white text-xs px-2 py-0.5 rounded hover:bg-toydb-orange-dark">Delete</button>
              </div>
            )) : <div className="text-sm text-toydb-slate">No photos</div>}
          </div>
        </div>

        {/* Bottom controls: Back, Delete Toy, Save Toy */}
        <div className="flex gap-2 justify-end mt-3">
          <button onClick={()=>navigate('/dashboard', { state: { refresh: Date.now() } })} className="border border-toydb-border bg-toydb-white text-toydb-navy p-2 rounded-lg">Cancel</button>
          <button onClick={deleteToy} disabled={busy} className="bg-toydb-danger text-toydb-white p-2 rounded-lg hover:bg-toydb-orange-dark">Delete Toy</button>
          <button onClick={save} disabled={busy} className="bg-toydb-teal text-toydb-white font-medium p-2 rounded-lg hover:bg-toydb-teal-dark">{busy ? 'Saving...' : 'Save Toy'}</button>
        </div>
      </div>
    </div>
  )
